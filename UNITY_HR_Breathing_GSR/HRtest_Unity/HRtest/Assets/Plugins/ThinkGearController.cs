...
public class ThinkGearController: MonoBehaviour {
	...
	void Update () {
		//nHeadsetConnectionRequest ();
		guiText.text = "Connect Status: " + connectStatus.ToString()+"PoorSignal: " + poorSignal.ToString("F2");//obj.HRV.ToString("F2") + "\n" + "Resistance: " + obj.resistance.ToString("F2") + " Conductance: " + obj.conductance.ToString("F2");
		}
  ...
  IEnumerator OnHeadsetConnectionRequest(){
    handleID = ThinkGear.TG_GetNewConnectionId();

    connectStatus = ThinkGear.TG_Connect(handleID,
		                                     Portname,
                                             baudRate,
                                             packetType);
  ...
  private void UpdateHeadsetData(){
    ...
    if(packetCount > 0){
      ...
      TriggerEvent("OnHeadsetDataReceived", values);
		...
		poorSignal = GetDataValue(ThinkGear.DATA_POOR_SIGNAL);
